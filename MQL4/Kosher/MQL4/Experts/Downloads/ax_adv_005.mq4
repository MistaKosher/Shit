...
int g_red3_ticket=-1;
int g_red2_ticket=-1;
input double g_profit=1.5;
input double g_loss=-0.5;
...
/*  if((ac==ACMODE_GREEN3 || ac==ACMODE_GREEN2) && iSAR(NULL,0,0.02,0.2,1)<g_ready_bar.low && ax_bar_utils::is_out_of_gator(g_ready_bar))
  ...
  if(ac==ACMODE_GREEN2 && g_green3_ticket>0 && g_green2_ticket<0 && 
     ax_bar_utils::get_bar_gator_position(g_ready_bar,BARPOSITION_ABOVEGATOR,BARPOSITIONMODE_FULL))//GREEN3 уже поставлен, GREEN2 еще нет
  {
   string err_msg;
  ...
  if(ac==ACMODE_RED2 && g_red3_ticket>0 && g_red2_ticket<0 &&
     ax_bar_utils::get_bar_gator_position(g_ready_bar,BARPOSITION_UNDERGATOR,BARPOSITIONMODE_FULL))//RED3 уже поставлен, RED2 еще нет
  {
   string err_msg;
