...
input int g_reversal_bar_cnt_wait=3;//количество баров для включения отложенного
input int g_direct_order_exp_bar_count=3;//время ожидания включения (прямой ордер),в барах
input int g_reverse_order_exp_bar_count=21;//время ожидания включения (обратный ордер),в барах
...
void OnTick()
{
  ...
  */
  ...
  ac_mode ac=ax_bar_utils::get_ac_mode3();
  //ставим GREEN3 + противоходный RED2
//#ifdef BUY
if(g_trade_mode==ADVTRADEMODE_BUY || g_trade_mode==ADVTRADEMODE_BOTH)
{
  if(ac==ACMODE_GREEN3 && //g_green3_ticket<0 &&
     ax_bar_utils::get_bar_gator_position(g_mqlrates,BARPOSITION_UNDERGATOR,BARPOSITIONMODE_PART3,1) &&
     !ax_bar_utils::inIchimokuCloud(TRADEMODE_BUY))
  {
   //buy
   //bool in_cloud=ax_bar_utils::inIchimokuCloud(TRADEMODE_BUY);
   ...
   int ticket=ax_bar_utils::OpenOrder3(dummy_bar,TRADEMODE_BUY,g_lots_3,g_slippage,"",g_direct_order_exp_bar_count,g_direct_profit_coef,err_msg);
   ...
   else
   {
    ...
    axAddOrder(ticket,dummy_bar.low,FIBO_618);
    
    //противоходный RED2
    ticket=ax_bar_utils::OpenOrder3(dummy_bar,TRADEMODE_SELL,g_lots_2,g_slippage,"",g_reverse_order_exp_bar_count,g_reverse_profit_coef,err_msg);
   
    if(ticket<0)
     msg+="[SELLSTOP]:"+err_msg;
    else
     axAddOrder(ticket,dummy_bar.high,FIBO_618);
   }
  }
//#endif 
}
...
//#ifdef SELL
  ...
  if(ac==ACMODE_RED3 && //g_red3_ticket<0 &&
     ax_bar_utils::get_bar_gator_position(g_mqlrates,BARPOSITION_ABOVEGATOR,BARPOSITIONMODE_PART3,1) &&
     !ax_bar_utils::inIchimokuCloud(TRADEMODE_SELL))
  {
   //sell
   string err_msg;
   ...
   int ticket=ax_bar_utils::OpenOrder3(dummy_bar,TRADEMODE_SELL,g_lots_3,g_slippage,"",g_direct_order_exp_bar_count,g_direct_profit_coef,err_msg);
   ...
   else
   {
    ...
    //противоходный GREEN2
    ticket=ax_bar_utils::OpenOrder3(dummy_bar,TRADEMODE_BUY,g_lots_2,g_slippage,"",g_reverse_order_exp_bar_count,g_reverse_profit_coef,err_msg);
